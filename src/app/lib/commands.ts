...
function link(
  label: string,
  href: string,
  color: OutputSegment["color"] = "cyan",
): OutputSegment {
...
function cmdHelp(theme: ThemeId): CommandResult {
  ...
  const rows: Array<[string, string]> = [
    ...
    [
      isCmd ? "cd" : n.pwd,
      isCmd ? "print current directory (no args)" : "print working directory",
    ],
  ...
  const slugNote = [
    ...
    line([
      text(
        "  archive slugs:    website, victoria-chorale, ock, jne, jio",
        "dim",
      ),
    ]),
    blank(),
  ...
  const aliasNote = isPsh
    ? [
        line([
          text(
            "  aliases: gci=Get-ChildItem  gc=Get-Content  gl=Get-Location  sl=Set-Location",
            "dim",
          ),
        ]),
        blank(),
...
function cmdWhoami(): CommandResult {
  return {
    lines: [
      ...
      line([
        text(
          "I specialise in building software to solve problems, but I especially enjoy automating tedious tasks and making quality of life enhancements. After all, software exist to make processes more efficient, and our lives easier.",
        ),
      ]),
      line([
        text(
          "My main stack is Python, but I am familiar with .Net, SQL and NextJS as well.",
        ),
      ]),
      blank(),
      line([
        text(
          "Looking for software development opportunities, but open to other related roles.",
        ),
      ]),
      blank(),
    ],
    ...
    for (const exp of experiences) {
      lines.push(
        line([
          text(`[${exp.date}]`, "dim"),
          text("  "),
          text(exp.role, "green", true),
          text(` @ ${exp.company}`, "white"),
        ]),
      );
  ...
  if (!exp) {
    return {
      lines: [
        ...
        line([
          text(
            `Valid slugs: ${experiences.map((e) => e.slug).join(", ")}`,
            "dim",
          ),
        ]),
      ],
...
function cmdLsProjects(): CommandResult {
  return {
    lines: [
      blank(),
      line(projects.map((p) => text(`${p.slug}.md  `, "cyan"))),
      blank(),
    ],
  };
}
...
function cmdCatProject(slug: string): CommandResult {
  ...
  if (!proj) {
    return {
      lines: [
        ...
        line([
          text(`Valid slugs: ${projects.map((p) => p.slug).join(", ")}`, "dim"),
        ]),
      ],
    };
  }
  ...
  const lines: OutputLine[] = [
    blank(),
    line([text(proj.title, "green", true)]),
  ];
  if (proj.link)
    lines.push(
      line([text("  link: ", "dim"), link(proj.link[1], proj.link[0])]),
    );
  lines.push(blank());
  for (const point of proj.points) lines.push(line([text(`  • ${point}`)]));
  lines.push(blank());
  lines.push(
    line([text("skills: ", "dim"), text(proj.skills.join(", "), "yellow")]),
  );
  lines.push(blank());
  lines.push(line([text("background:", "dim")]));
  for (const detail of proj.details) {
    lines.push(line([text(`  ${detail}`)]));
    lines.push(blank());
  }
  if (proj.screenshots.length > 0) {
    lines.push(line([text("screenshots:", "dim")]));
    for (const src of proj.screenshots)
      lines.push(line([text(`  ${src}  `, "dim"), link("[open ↗]", src)]));
    lines.push(blank());
  }
  return { lines };
}
...
function cmdLsArchives(): CommandResult {
  return {
    lines: [
      blank(),
      line(archives.map((a) => text(`${a.slug}.md  `, "cyan"))),
      blank(),
    ],
  };
}
...
function cmdCatArchive(slug: string): CommandResult {
  ...
  if (!arch) {
    return {
      lines: [
        ...
        line([
          text(`Valid slugs: ${archives.map((a) => a.slug).join(", ")}`, "dim"),
        ]),
      ],
    };
  }
  ...
  return {
    lines: [
      ...
      ...arch.screenshots.map((src) =>
        line([text(`  ${src}  `, "dim"), link("[open ↗]", src)]),
      ),
      blank(),
    ],
  };
...
function cmdLs(currentPath: VirtualPath): CommandResult {
  ...
  return {
    lines: [
      ...
      line(
        items.map((item) =>
          text(`${item}  `, item.endsWith("/") ? "cyan" : "white"),
        ),
      ),
      blank(),
    ],
...
function cmdContact(): CommandResult {
  return {
    lines: [
      ...
      line([
        text("  email:    ", "dim"),
        link("lim.jia.le51@gmail.com", "mailto:lim.jia.le51@gmail.com"),
      ]),
      line([
        text("  linkedin: ", "dim"),
        link(
          "linkedin.com/in/lim-jia-le/",
          "https://linkedin.com/in/lim-jia-le/",
        ),
      ]),
      line([
        text("  github:   ", "dim"),
        link("github.com/jaey8den", "https://github.com/jaey8den"),
      ]),
      line([
        text("  resume:   ", "dim"),
        link("LimJiaLe-Resume.pdf [download]", "/LimJiaLe-Resume.pdf"),
      ]),
      blank(),
    ],
  };
...
function cmdBanner(): CommandResult {
  return {
    lines: [
      blank(),
      ...BANNER.split("\n").map((l) =>
        line([{ text: l, color: "green" as const, small: true }]),
      ),
      blank(),
    ],
  };
...
function cmdExit(): CommandResult {
  return {
    lines: [
      blank(),
      line([text("You can't leave. This is my portfolio.", "yellow")]),
      blank(),
    ],
  };
}
...
function cmdSudo(args: string): CommandResult {
  return {
    lines: [
      line([text(`sudo: ${args}: permission denied — nice try.`, "red")]),
    ],
  };
}
...
function resolveCatArgs(args: string, currentPath: VirtualPath): CommandResult {
  ...
  if (catTarget.startsWith("experience/"))
    return cmdCatExperience(catTarget.replace("experience/", ""));
  if (catTarget.startsWith("projects/"))
    return cmdCatProject(catTarget.replace("projects/", ""));
  if (catTarget.startsWith("archives/"))
    return cmdCatArchive(catTarget.replace("archives/", ""));
  ...
  return {
    lines: [
      ...
      line([
        text(
          "Try: cat experience.md, cat projects/cdsm.md, cat archives/jio.md",
          "dim",
        ),
      ]),
    ],
...
export function executeCommand(
  ...
  switch (cmdLower) {
    ...
    case "open": {
      if (!args.trim())
        return { lines: [line([text("open: missing argument", "red")])] };
      if (typeof window !== "undefined")
        window.open(args.trim(), "_blank", "noopener,noreferrer");
